...
- `UNABLE_TO_VERIFY` is a risk level, not a numeric score shortcut. It should be used when evidence quality is too low for a reliable category or overall assessment.
...
- If no detector findings are available to score, no `RiskAssessment` is persisted and public interfaces report `UNABLE_TO_VERIFY` with `score: null`.
