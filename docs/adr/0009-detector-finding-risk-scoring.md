...
Stage 6 exposed persisted scan findings through API, web, and Telegram, but risk snapshots remained `UNABLE_TO_VERIFY` because no scoring engine persisted `RiskAssessment` records.
...
If there are no findings to score, the scorer returns no assessment. The worker records `SCORING` as `SKIPPED`, and public interfaces continue to report `UNABLE_TO_VERIFY` with `score: null`.
